...
from utils.BCP_utils import context_mask_2D, mix_loss, parameter_sharing, update_ema_variables
from dataloaders.dataset import (BaseDataSets, ReconsitionDataSets, RandomGenerator, TwoStreamBatchSampler)
...
# 读取图像文件列表
directory = '/home/wangjh/project/CC-Net-V2/data/ACDC/reconstruction'
fname_list = [os.path.splitext(file)[0] for file in os.listdir(directory) if file.endswith('.h5')]
output_directory = '/home/wangjh/project/CC-Net-V2/data/ACDC'
output_file = os.path.join(output_directory, 'Recons.list')
...
def train(args, snapshot_path):
    ...
    db_train1 = ReconsitionDataSets(base_dir=args.root_path, 
                            split="Recons", 
                            num=None, 
                            transform=transforms.Compose([
        RandomGenerator(args.patch_size)
    ...
    db_val = BaseDataSets(base_dir=args.root_path, split="val")
     # 合并数据集
    db_combined = ConcatDataset([db_train, db_train1])
    total_slices = len(db_combined)
    ...
    trainloader = DataLoader(db_combined, batch_sampler=batch_sampler, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    model.train()
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            for idx in range(num_outputs):
                ...
                loss_ce = F.cross_entropy(y1,label_batch1[:labeled_bs][:].long())
                loss_dice = dice_loss(y_prob1,label_batch1[:labeled_bs].unsqueeze(1))
                loss1 = (loss_ce + loss_dice) / 2    
                loss_seg_dice +=loss1*2.3
                y_all = outputs[idx]
