...
parser.add_argument('--exp', type=str, default='DACNet', help='exp_name')
parser.add_argument('--model', type=str, default='dacnet3d_v1', help='model_name')
parser.add_argument('--max_iteration', type=int, default=16000, help='maximum iteration to train')
...
if __name__ == "__main__":
    ...
    def create_model(ema=False):
        # Network definition
        net =net_factory(net_type="vnet", in_chns=1, class_num=num_classes, mode="train")
        model = net.cuda()
        if ema:
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for idx in range(num_outputs):
                ...
                loss_ce = F.cross_entropy(y_prob1[:labeled_bs], label_batch1[:labeled_bs])
                loss_dice = dice_loss(y_prob1[:, 1, ...], label_batch1[:labeled_bs, ...] == 1)
                loss1 = (loss_ce + loss_dice) / 2    
                loss_seg_dice +=loss1*1.9
            ...
            loss.backward()
            optimizer.step()
            update_ema_variables(model.encoder1, ema_model.encoder, args.ema_decay, iter_num)
            update_ema_variables(model.decoder1, ema_model.decoder1, args.ema_decay, iter_num)
